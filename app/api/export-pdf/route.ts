      ...
      if (cikRes.ok) {
        ...
        if (entry) {
          ...
          if (secRes.ok) {
            ...
            // ✅ Fix: use stateOrCountryDescription first
    ...
    try {
      ...
      if (promoRes.ok) {
        ...
        promotions = rawPromos.map((p: any) => ({
          ...
          url:
            p.url ||
            `https://www.stockpromotiontracker.com/stocks/${upperTicker}`,
        }));
      }
    } catch (err) {
      ...
      promotions = [
        {
          type: "Manual Check",
          date: "",
          url: `https://www.stockpromotiontracker.com/stocks/${upperTicker}`,
        },
      ];
    }
